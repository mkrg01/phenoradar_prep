...
def test_existing_odb_rejects_incompatible_inputs(existing_odb, tmp_path, problem, message):
    ...
    elif problem in {"node", "version", "schema_version"}:
        snapshot[problem] = "wrong"
    elif problem == "duplicate":
        snapshot["proteins"].append(snapshot["proteins"][0])
    elif problem == "odb_species":
        snapshot["proteins"][0]["odb_species"] = "Wrong_species"
    elif problem == "path":
        snapshot["annotations"]["path"] = "../annotations.tsv"
    else:
...
def test_snakemake_end_to_end_and_incremental_rerun(tiny_inputs, fake_odb, frozen_reference, existing_odb, tmp_path, reuse, command_environment, workflow_project, seed_taxonomy):
    ...
    if reuse:
        ...
        assert not (workflow_project / "resources/orthodb").exists()
