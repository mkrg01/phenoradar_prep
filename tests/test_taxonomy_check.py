...
from taxonomy_check import Taxonomy, check, parse_tree, validate_settings
...
def run_check(inputs, out, **settings):
    require_monophy()
...
def test_native_roles_run_linkage_and_report_replacement(check_inputs, tmp_path):
    ...
    assert {k: sha256(v) for k, v in check_inputs.items()} == before
    ...
    subprocess.run([shutil.which("Rscript"), "--vanilla", "-e", code, str(check_inputs["tree"]),
    ...
    run_check(check_inputs, out, outlierlevel=0.4)
    assert len(read_tsv(out / "candidates.tsv")) > 2
    check_inputs["tree"].write_text(RIGHT + "\n")
    result = run_check(check_inputs, out)
    assert result["candidate_species"] == 0 and read_tsv(out / "candidates.tsv") == []
    assert not (out / "ranks/subfamily").exists()


def test_missing_rank_is_not_an_intruder_and_all_flags_stay_in_plot(check_inputs, tmp_path):
    with sqlite3.connect(check_inputs["taxonomy"]) as db:
        db.execute("UPDATE species SET track='101,5,1' WHERE taxid=101")
    out = tmp_path / "check"
    summary = run_check(check_inputs, out)
...
def test_grouped_intruders_are_detected_by_monophy(check_inputs, tmp_path):
    ...
    check_inputs["tree"].write_text(text)
    out = tmp_path / "check"
    run_check(check_inputs, out)
...
def test_rank_with_one_annotated_tip_is_reported_without_native_assessment(check_inputs, tmp_path):
    with sqlite3.connect(check_inputs["taxonomy"]) as db:
    ...
    out = tmp_path / "check"
    summary = run_check(check_inputs, out, ranks=["subfamily"])
    assert summary["candidate_species"] == 0
...
def test_failed_validation_preserves_report(check_inputs, tmp_path):
    out = tmp_path / "check"
    run_check(check_inputs, out)
    before = {p.relative_to(out): sha256(p) for p in out.rglob("*") if p.is_file()}
    report = json.loads(check_inputs["tree_qc"].read_text())
    report["outgroup"] = "A_one"
    write_json(check_inputs["tree_qc"], report)
    ...
    with pytest.raises(ValueError, match="overlaps"):
        check(**check_inputs, outdir=check_inputs["tree"].parent)


def test_manifest_conflicts_fail(check_inputs, tmp_path):
    rows = read_tsv(check_inputs["samples"])
    rows[-1]["taxid"] = "9999"
    write_tsv(check_inputs["samples"], list(rows[0]), rows)
    with pytest.raises(ValueError, match="conflicting species"):
        check(**check_inputs, outdir=tmp_path / "check")


def test_frozen_taxonomy_merged_ids(check_inputs):
    taxonomy = Taxonomy(check_inputs["taxonomy"])
    try:
...
@pytest.mark.parametrize("settings", [{"min_reference_species": 2}, {"max_plot_species": 24}, {"mem_gb": 8},
...
def test_snakemake_reuses_species_tree_without_gene_inputs(check_inputs, workflow_project, command_environment):
    require_monophy()
    ...
    shutil.copyfile(check_inputs["samples"], metadata / "samples.tsv")
    taxdir = project / "resources/taxonomy"
    taxdir.mkdir(parents=True)
    shutil.copyfile(check_inputs["taxonomy"], taxdir / "taxa.sqlite")
    ...
    for branch in ["phylogeny/all", "phylogeny/phenotyped"]:
        ...
        for key in ["tree", "tree_qc"]:
            dest = folder / check_inputs[key].name
            shutil.copyfile(check_inputs[key], dest)
    ...
    wrapper.write_text("\n".join(line for line in (ROOT / "workflow/Snakefile").read_text().splitlines()
                                 if not line.startswith("include:") or '"rules/taxonomy_check.smk"' in line) + "\n")
    argv = [snakemake, "--snakefile", str(wrapper), "--configfile", str(override), "--cores", "1", "--"]
    def run(target="taxonomy_check"):
    ...
    full = project / "results/test/phylogeny/all/taxonomy_check"
    phenotyped = project / "results/test/phylogeny/phenotyped/taxonomy_check"
